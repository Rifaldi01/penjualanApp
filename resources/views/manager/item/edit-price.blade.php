...
@section('content')
    <div class="card">
        ...
        @if ($errors->any())
            ...
            @endforeach
        @endif
        <form class="card-body p-4" action="{{route('manager.item.updatePrice', $item->no_seri)}}" method="POST" enctype="multipart/form-data" id="myForm">
            @csrf
                @method('PUT')
            <div class="row">
                <div class="col-sm-6">
                ...
                <div class="col-sm-3">
                    <div class="mb-2">
                        ...
                        <input type="text" name="kode_msk" class="form-control" placeholder="Enter Invoice"
                               value="{{ isset($item) ? optional($item)->kode_msk : null }}">
                    </div>
                </div>
            </div>
            <div class="row">
                    ...
                    <div class="mt-3 mb-2">
                        ...
                        <select name="divisi_id" class="form-select" id="single-select-clear-field"
                            ...
                            @foreach($divisi as $div)
                                @if(isset($item))
                                    <option
                                        value="{{ $div->id }}" {{ $item->divisi_id == $div->id ? 'selected' : '' }}>{{ $div->name }}</option>
                                @else
                                    <option value=""></option>
