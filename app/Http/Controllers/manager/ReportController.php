...
class ReportController extends Controller
{
    public function index()
    {
        ...
        $divisi = Divisi::whereNotIn('name', ['Rental', 'rental'])
            ->where('status', 'active')
            ->get();
        ...
        $report->each(function ($sale) {
            ...
            $sale->debt = $sale->debt
                ? $sale->debt->map(function ($debt) {
                    $bankOrDescription = $debt->bank?->name ?? $debt->description ?? 'Tidak ada informasi';
                    return $debt->date_pay . ' (' . $bankOrDescription . ') ' . $debt->pay_debts . ' ' . $debt->penerima;
                })->implode(', ')
                : '-';
        });
